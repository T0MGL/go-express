-- 056: el ingreso por mostrador no era representable. Cuando el cliente trae el paquete a
-- la sucursal, el paquete YA esta en el deposito: 'recolectado' y 'en_transito' nunca
-- ocurrieron. La matriz solo permitia pendiente -> {recolectado, problema}, asi que
-- warehouse.service.ts:164 intentaba pasar a 'en_deposito', la base lo rechazaba con
-- transicion_invalida y el operador veia el ingreso como exitoso mientras el envio seguia
-- 'pendiente'. Se agrega 'en_deposito' a las transiciones validas desde 'pendiente'.
--
-- Recrea la funcion viva (045) tal cual, cambiando solo esa linea, y honra:
...
BEGIN
  ...
  v_allowed := CASE v_envio_previo.estado
    WHEN 'pendiente'    THEN ARRAY['recolectado', 'en_deposito', 'problema']::envio_estado[]
...
$function$;
COMMIT;
-- ROLLBACK: re-aplicar 045_entregado_admin_fecha_entrega_real.sql, que tiene la misma
-- definicion sin 'en_deposito' en la fila de 'pendiente'. La funcion es la unica entidad
